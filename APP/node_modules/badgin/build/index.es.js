...
const drawFavicon = (image, value, options) => {
    if (!current$1.canvas || !current$1.context) {
        return;
    }
    // Clear old canvas
    current$1.context.clearRect(0, 0, size, size);
    // Draw new image
    current$1.context.drawImage(image, 0, 0, image.width, image.height, 0, 0, size, size);
    // Draw bubble on the top
    drawBubble(current$1.context, value, options);
    // Refresh tag in page
    setFavicon(current$1.canvas.toDataURL());
};
...
function isAvailable$1() {
    if (!current$1.context) {
        const canvas = document.createElement('canvas');
        canvas.width = size;
        canvas.height = size;
        current$1.canvas = canvas;
        current$1.context = canvas.getContext ? canvas.getContext('2d') : null;
    }
    return !!current$1.context && !!getBestFavicon();
}

...
const SelecionaProduto = props => {
    ...
    const [produtoTmp, setProdutoTmp] = useState( produto );
  ...
  async function buscaNome(){
    ...
    let id = produtoTmp.id;
    let codigo_livro = produtoTmp.codigo;
    let nome_livro = produtoTmp.nome;
    let autor = produtoTmp.autor;
    let genero = produtoTmp.genero;
    let colecao = produtoTmp.colecao;
    let edicao = produtoTmp.edicao;
    let observacao = produtoTmp.observacao;
    ...
    const [date,setNewDate]=useState(created_at) ;
     ...
     async function submit(){
        await api.post('/src/all/emprestimo',
        
        {codigo_livro,nome_livro,codigo_visitante,nome_visitante,data_emprestimo,dias_emprestimo,data_devolucao} 
    )
    .then(async function(response){
    ...
    .catch(function(error){
        ...
        try{
            ...
            error.response.data.message !== "undefined" ? 
            error.response.data.message : 
            "Houve uma falha ao tentar fazer o Empréstimo do Livro Verifique o Nome do Visitante e o Código dele";
            Alert.alert("Erro", message);
            
        
        }catch(err){
            
            props.navigation.navigate('Erro');
        ...
        <View style={{flex:1, justifyContent:"space-between"}}>
            ...
            <ScrollView style={styles.containerScrollView}>
                ...
                >
                    ...
                    <Text style={styles.textdashboard}>Empréstimo de Livros</Text>
                    <Text style={styles.textdashboard}>Emprestando o Livro {codigo_livro} - {nome_livro}</Text>
                    ...
                    <View style={styles.viewTextDashboard1}>
                        ...
                        <Picker
                            ...
                            backgroundColor: 'white',padding: 10,color:'black',
                            fontSize: 14,
                            borderRadius: 8,
                            elevation: 7 }}
                        ...
                        onPress={submit} 
                        >
                            <Text style={styles.submitText}>Finalizar Empréstimo</Text>
